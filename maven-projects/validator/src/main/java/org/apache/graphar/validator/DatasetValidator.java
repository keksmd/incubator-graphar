...
public final class DatasetValidator {
    ...
    private interface RowCheck {
        void check(RecordBatch batch, int row, URI uri);
    }
    ...
    private static final class TopologyCheck implements RowCheck {
        ...
        @Override
        public void check(RecordBatch batch, int row, URI uri) {
            Object source = batch.column(0).getObject(row);
            Object destination = batch.column(1).getObject(row);
