...
Phase 0 documents live under `docs/libraries/java/` and are linked from the Java library overview. The upstream delivery chain is issue-first: architecture discussion [#756](https://github.com/apache/incubator-graphar/issues/756), edge-layout gate [#943](https://github.com/apache/incubator-graphar/issues/943), and metadata parity [#944](https://github.com/apache/incubator-graphar/issues/944). Phase 1a makes `EdgeInfo` C++-compatible: counts and offsets resolve below the adjacency prefix, topology resolves as `adj_list/part{vertexChunk}/chunk{edgeChunk}`, and edge properties resolve below the selected adjacency layout with the same tuple. The test suite proves these suffixes against the real Parquet fixture, including `part2/chunk1`, and covers prefixes without trailing slashes. The Java metadata fixture now emits `list<string>` as well as labels and extra metadata. The resulting full devcontainer `mvn clean verify` passed 128 tests with zero failures, errors, or skips. Its C++-loader acceptance is blocked only by [#945](https://github.com/apache/incubator-graphar/issues/945): the C++ devcontainer build fails against its packaged Arrow API before it can read this fixture.
